...
async function identifyContent(query, originalUrl) {
    ...
    return [
      {
        ...
        poster: '',
        streamingSources,
        confidence: result.confidence === 'high' ? 0.95 : result.confidence === 'medium' ? 0.8 : 0.6,
...
async function enrichWithPosters(results) {
  ...
  for (const result of results){
    try {
      ...
      if (tmdbResults.length > 0) {
        ...
        if (result.year) {
          const yearMatch = tmdbResults.find((r)=>{
            const releaseYear = searchType === 'movie' ? r.release_date?.split('-')[0] : r.first_air_date?.split('-')[0];
...
async function enrichWithStreamingData(results) {
  for (const result of results){
    if (result.type === 'youtube') {
      ...
      try {
        ...
        if (searchData && searchData.length > 0) {
          matchedShow = searchData.find((show)=>{
        ...
        if (matchedShow.streamingOptions && matchedShow.streamingOptions.us) {
          for (const option of matchedShow.streamingOptions.us){
            ...
            const streamingSource = {
              ...
              url: option.link,
              type: option.type === 'subscription' ? 'subscription' : option.type === 'rent' ? 'rent' : option.type === 'buy' ? 'buy' : 'free',
...
async function fetchYouTubeData(videoTitle) {
  ...
  try {
    ...
    const result = {
      thumbnail: videoDetails.snippet.thumbnails.maxres?.url || videoDetails.snippet.thumbnails.high?.url || videoDetails.snippet.thumbnails.medium?.url || videoDetails.snippet.thumbnails.default?.url,
      url: `https://www.youtube.com/watch?v=${videoId}`,
...
function detectStreamingService(url) {
  const services = [
    ...
    {
      pattern: /primevideo\.com|amazon\.com/,
      id: 'prime',
      name: 'Prime Video'
    },
    {
    ...
    {
      pattern: /peacocktv\.com/,
      id: 'peacock',
      name: 'Peacock'
    },
    {
      pattern: /paramountplus\.com/,
      id: 'paramount',
      name: 'Paramount+'
    },
    {
...
function generateSearchUrl(serviceId, title) {
...
function generateFallbackSources(title) {
  ...
  const selectedServices = fallbackServices.sort(()=>Math.random() - 0.5).slice(0, Math.floor(Math.random() * 2) + 2);
